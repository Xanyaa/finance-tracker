...
if (isset($_SESSION['Id'], $_SESSION['Username'])) {
    ...
    $expenseData = [];
    $query = "SELECT received_dt, amount FROM expense WHERE user_id = ?";
...
<body class="fix-header fix-sidebar">
                ...
                <div class="col-lg-12">
                        <div class="card">
                            <div class="card-block">
                                <h4 class="card-title">Expense Chart</h4>
    ...
    <script>
        // Convert PHP expense data to JavaScript array
        var expenseData = <?php echo json_encode($expenseData); ?>;

        // Prepare data for Chart.js
        var chartData = {
            labels: expenseData.map(item => item.received_dt),
            datasets: [{
                label: 'Amount',
            ...
            scales: {
                ...
                y: [{
                    ticks: {
                        beginAtZero: true,
                        suggestedMin: 0,
                        suggestedMax: Math.max(...expenseData.map(item => item.amount)) + 200,
                        stepSize: 20,
                        maxTicksLimit: 10,
                    },
                    gridLines: {
