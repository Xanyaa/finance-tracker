...
if (isset($_SESSION['Id'], $_SESSION['Username'])) {
    ...
    // Fetch income data
    $incomeQuery = "SELECT received_dt, amount FROM income WHERE user_id = ?";
    ...
    while ($row = $incomeResult->fetch_assoc()) {
        $incomeData[$row['received_dt']][] = $row['amount'];
    }

    $incomeStmt->close();

    // Fetch expense data
    $expenseQuery = "SELECT received_dt, amount FROM expense WHERE user_id = ?";
    ...
    while ($row = $expenseResult->fetch_assoc()) {
        $expenseData[$row['received_dt']][] = $row['amount'];
    }
    ...
    $balanceData = [];
    foreach ($incomeData as $date => $incomeAmounts) {
        $balanceData[$date] = array_sum($incomeAmounts) - (isset($expenseData[$date]) ? array_sum($expenseData[$date]) : 0);
    }
} else {
...
<body class="fix-header fix-sidebar">
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-block">
                            <h4 class="card-title">Balance Chart</h4>
    ...
    <script>
        // Convert PHP balance data to JavaScript array
        var balanceData = <?php echo json_encode($balanceData); ?>;
        ...
        ctx.canvas.width = 1000; // Adjust as needed
        ctx.canvas.height = 400; // Adjust as needed
